...
class HeroTestCase(TestCase):
    ...
    def test_hero_list_wrongmethod(self):
        ...
        response = self.client.put('/api/hero', json.dumps({'name': 'Spiderman'}), content_type='application/json')

        self.assertEqual(response.status_code, 405)

        # Test heroList with DELETE request
        response = self.client.delete('/api/hero', json.dumps({'name': 'Spiderman'}), content_type='application/json')
